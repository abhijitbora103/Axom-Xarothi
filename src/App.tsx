...
export default function App() {
  // Localization state: 'en' for English, 'as' for Assamese (অসমীয়া)
  ...
  // Scroll logic for chat message logs
  useEffect(() => {
  ...
  const handleSearchShortcut = (term: string) => {
    setSearchQuery(term);
    setActiveTab("all");
    setSelectedItem(null); // return to home context
  ...
  // --- Dynamic Headline Tickers based on Language Selection ---
  ...
  const filteredUpdates = ASSAM_UPDATES.filter((item) => {
  ...
  // --- secure API Route Chat submit proxy ---
    ...
    try {
      ...
      if (!res.ok) {
        throw new Error(lang === "en" 
          ...
          : "সহায়িকা চাৰ্ভাৰৰ সৈতে সংযোগ ব্যৰ্থ হৈছে।"
        );
      }
  ...
  const handleAskAIAboutItem = (item: UpdateItem) => {
    ...
    const query = lang === "en"
      ...
      : `মোক নিম্নলিখিত বিষয়ে বিশদ তথ্য দিয়া চমু অৰ্হতাসহ: "${item.titleAs}"।`;
  ...
  const handleCopyLink = (id: string) => {
    ...
    triggerToast(lang === "en" ? "Sharable link copied to clipboard!" : "শ্বেয়াৰ লিংক ক্লিপবোৰ্ডত সংৰক্ষণ হ’ল!");
  };

  // --- Exporter Codes Mock Sources ---
  const htmlCodeSample = `<!-- Copy index.html from /public/static-dist/index.html directory -->
<!-- Directly hosted for you! File path: /public/static-dist/index.html -->
<html lang="en">
...
/* Features silky breaking news marquee & custom dark modes transition curves */`;

  const jsCodeSample = `// Copy script.js from /public/static-dist/script.js directory
// Houses the entire bilingual datasets, custom FAQ accordions, and offline chatbot!`;
  ...
  return (
    <div className="bg-slate-50 text-slate-800 dark:bg-slate-900 dark:text-slate-100 min-h-screen flex flex-col font-sans transition-colors duration-300">
      ...
      <header className="bg-white dark:bg-slate-950 shadow-xs border-b border-slate-150 dark:border-slate-850 sticky top-0 z-40 transition-colors">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between items-center h-20">
            ...
            >
              <div className="w-11 h-11 bg-emerald-750 bg-emerald-700 text-white rounded-2xl flex items-center justify-center shadow-md shadow-emerald-700/20 group-hover:scale-105 transition-transform">
            ...
            <div className="flex items-center space-x-2">
              ...
              {/* ZIP STATIC EXPORTER MODULE TRIGGER */}
              <button 
              ...
              <button 
                ...
                className="p-2.5 bg-slate-100 hover:bg-slate-200 dark:bg-slate-900 dark:hover:bg-slate-850 rounded-xl text-slate-605 dark:text-slate-200 active:scale-95 transition-all border border-transparent dark:border-slate-800 cursor-pointer"
        ...
        {mobileMenuOpen && (
          <div className="md:hidden border-t border-slate-150 dark:border-slate-800 bg-white dark:bg-slate-950 p-4 space-y-2 max-h-[80vh] overflow-y-auto">
            <a href="#" onClick={(e) => { e.preventDefault(); setSelectedItem(null); setActiveTab("all"); setMobileMenuOpen(false); }} className="block px-4 py-2.5 rounded-xl text-xs font-extrabold bg-slate-50 dark:bg-slate-900 text-slate-850 dark:text-white">
            ...
            <a href="#" onClick={(e) => { e.preventDefault(); setSelectedItem(null); setActiveTab("recruitment"); setMobileMenuOpen(false); }} className="block px-4 py-2.5 rounded-xl text-xs font-bold text-slate-600 dark:text-slate-300">
              {lang === "en" ? "Jobs & Vacancies (চাকৰি)" : "চাকৰি আৰু নিয়োজন"}
            </a>
      ...
      {/* DYNAMIC METADATA ROUTED TO MAIN CONTENT SCREEN or BILINGUAL HERO */}
      {!selectedItem ? (
        <main className="flex-grow max-w-7xl mx-auto w-full px-4 sm:px-6 lg:px-8 py-8">
          
          {/* SEARCH & PROMPT BANNER FOR THE USER */}
              ...
              <div className="max-w-3xl relative z-10">
                ...
                <div className="mt-8 flex flex-col sm:flex-row gap-3">
                  <div className="relative flex-grow">
                    ...
                    {searchQuery && (
                      <button 
                        ...
                        className="absolute right-4 top-1/2 -translate-y-1/2 text-xs font-bold text-slate-400 hover:text-slate-805 uppercase"
                      >
                        {lang === "en" ? "Clear" : "মচি দিয়ক"}
                      </button>
                    )}
                  </div>
                  ...
                  <button 
                    ...
                    className="bg-emerald-650 bg-emerald-700 hover:bg-emerald-800 text-white font-extrabold px-6 py-4 rounded-2xl text-xs sm:text-sm tracking-wider shadow-md transition-all active:scale-95 cursor-pointer text-center"
                  >
                ...
                <div className="mt-5 flex flex-wrap items-center gap-2 text-xs">
                  <span className="text-slate-400 font-extrabold uppercase text-[10px] tracking-wider">
                    {lang === "en" ? "QUICK BADGES:" : "অধিক সোধা বিষয়:"}
                  </span>
          ...
          {/* SPOTLIGHT FEATURED HERO - DYNAMICALLY HIGHLIGHTED FROM DATASET */}
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
              ...
              <div className="lg:col-span-2 bg-gradient-to-br from-indigo-50/40 to-emerald-50/20 dark:from-slate-950 dark:to-slate-900 border border-slate-200 dark:border-slate-800 p-6 sm:p-10 rounded-3xl flex flex-col justify-between shadow-xs relative overflow-hidden group hover:shadow-lg transition-transform duration-350">
                ...
                <div>
                  ...
                  <h4 className="text-xl sm:text-3xl font-black text-[#001f3f] dark:text-white leading-snug group-hover:text-emerald-700 dark:group-hover:text-emerald-400 transition-colors">
                    {lang === "en" 
                      ...
                      : "ADRE ৩য় আৰু ৪র্থ শ্ৰেণীৰ লিখিত পৰীক্ষাৰ দিন ঘোষণা কৰা হৈছে"}
                  </h4>

                  <p className="text-slate-650 dark:text-slate-350 text-xs sm:text-sm mt-3 leading-relaxed max-w-2xl">
                    {lang === "en"
                      ...
                      : "ৰাজ্যিক পৰ্যায়ৰ নিযুক্তি আয়োগে ৩য় আৰু ৪র্থ বৰ্গৰ লিখিত পৰীক্ষাৰ বাবে বিশদ সময়সূচীৰ খতিয়ান আনুষ্ঠানিকভাবে মুকলি কৰিছে। পৰীক্ষা নিয়মাৱলী আৰু চিলেবাচ সবিশেষ ইয়াতে পাব।"}
                  </p>
                </div>
                ...
                <div className="mt-8 flex items-center justify-between gap-4">
                  <button 
                    onClick={() => {
                      const item = ASSAM_UPDATES.find(u => u.id === "adre-recruitment-2026") || ASSAM_UPDATES[0];
                  ...
                  >
                    <span>{lang === "en" ? "Read Full Article" : "পূৰ্ণ গাইডলাইন পঢ়ক"}</span>
              ...
              {/* Right secondary alerts */}
              <div className="flex flex-col gap-4">
                
                {/* Secondary spotlight card 1 */}
                <div 
                  onClick={() => setSelectedItem(ASSAM_UPDATES.find(u => u.id === "orunodoi-scheme-2026") || null)}
                  className="bg-white dark:bg-slate-950 border border-slate-200 dark:border-slate-800/80 p-5 rounded-3xl hover:border-emerald-650/40 dark:hover:border-emerald-400/40 transition-all cursor-pointer flex flex-col justify-between hover:shadow-md h-full"
                >
                  <div>
                    <span className="inline-flex bg-emerald-55 bg-emerald-50 text-emerald-800 dark:bg-emerald-950/20 dark:text-emerald-400 border border-emerald-100 dark:border-emerald-900 text-[9px] font-black px-2.5 py-0.5 rounded-full uppercase">
                ...
                {/* Secondary spotlight card 2 */}
                <div 
                  onClick={() => setSelectedItem(ASSAM_UPDATES.find(u => u.id === "pragyan-free-scooty") || null)}
                  className="bg-white dark:bg-slate-950 border border-slate-200 dark:border-slate-800/80 p-5 rounded-3xl hover:border-emerald-650/40 dark:hover:border-emerald-400/40 transition-all cursor-pointer flex flex-col justify-between hover:shadow-md h-full"
                >
                  <div>
                    <span className="inline-flex bg-indigo-55 bg-indigo-50 text-indigo-805 dark:bg-indigo-950/20 dark:text-indigo-400 border border-indigo-100 dark:border-indigo-900 text-[9px] font-black px-2.5 py-0.5 rounded-full uppercase">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            <div className="lg:col-span-2 space-y-6">
              ...
              <div className="flex items-center justify-between border-b border-slate-200 dark:border-slate-850 pb-3 flex-wrap gap-2">
                ...
                <span className="bg-slate-100 dark:bg-slate-900 text-slate-650 dark:text-slate-350 text-xs font-black px-3 py-1 rounded-full border border-slate-200 dark:border-slate-800">
                  {filteredUpdates.length} {lang === "en" ? "Articles" : "বুলেটিন"}
                </span>
              </div>

              {/* HORIZONTAL SLIDER CATEGORY PILLS */}
                  ...
                  return (
                    <button
                      ...
                      onClick={() => {
                        ...
                        // scroll quiz center focus if examprep
                      ...
                      className={`px-4 py-2.5 rounded-xl text-xs sm:text-sm font-bold whitespace-nowrap transition-all border ${
                        isSelected 
                          ? "bg-[#001f3f] text-white border-[#001f3f] dark:bg-slate-850 dark:border-slate-800" 
                      ...
                      >
                        <div className="p-5">
                          ...
                          <div className="mt-4 pt-3.5 border-t border-slate-100 dark:border-slate-900/60 space-y-1">
                            ...
                            <div className="flex items-start text-xs text-slate-705 dark:text-slate-350">
            ...
            {/* RIGHT COLUMN: RICH SIDEBAR */}
            <aside className="space-y-6">
              
              {/* TRENDING & POPULAR POSTS INDICATORS */}
              <div className="bg-white dark:bg-slate-950 border border-slate-200 dark:border-slate-800 p-6 rounded-3xl shadow-xs">
                ...
                <ul className="space-y-4">
                  {[
                    ...
                    const matchingUpdate = ASSAM_UPDATES.find(u => u.id === trendingItem.id);
                    return (
                      <li 
                        key={trendingItem.id}
                        onClick={() => {
                      ...
                      >
                        ...
                        <div>
                          <h5 className="text-xs sm:text-sm font-bold text-slate-800 dark:text-slate-205 group-hover:text-emerald-700 dark:group-hover:text-emerald-400 transition-colors line-clamp-2">
                            {trendingItem.title}
                          </h5>
              ...
              {/* ASSAM GENERAL GK RECALL PRACTICES */}
                ...
                {!quizCompleted ? (
                  <div>
                    <div className="flex justify-between items-center text-[10px] text-slate-400 uppercase font-black mb-2.5">
                      ...
                      <span className="bg-emerald-50 text-emerald-850 px-2 py-0.5 rounded font-black dark:bg-slate-900 dark:text-emerald-400">Score: {score}</span>
                    </div>
                    ...
                    <div className="mt-4 space-y-2">
                      {(lang === "en" ? MOCK_QUIZ_QUESTIONS[currentQuestionIndex].options : MOCK_QUIZ_QUESTIONS[currentQuestionIndex].optionsAs).map((opt, idx) => {
                        ...
                        let styleClasses = "bg-slate-50 hover:bg-slate-100 border-slate-200 text-slate-800 dark:bg-slate-900 dark:hover:bg-slate-850 dark:border-slate-800 dark:text-slate-200";
                        if (isAnswered) {
                          if (isCorrectOption) {
                            ...
                            styleClasses = "bg-red-100 border-red-400 text-red-900 dark:bg-red-950/40 dark:text-red-300 dark:border-red-850";
                          } else {
                            styleClasses = "bg-slate-50/40 border-slate-150 text-slate-400 dark:bg-slate-900/10 dark:text-slate-600 dark:border-slate-850 pointer-events-none";
                          }
                        }
                    ...
                    {selectedAnswerIndex !== null && (
                      <div className="mt-4 p-4 rounded-xl bg-slate-100 dark:bg-slate-900/60 border border-slate-200 dark:border-slate-800 animate-fadeIn">
                        <p className="text-[11px] leading-relaxed text-slate-600 dark:text-slate-350 font-sans font-medium">
              ...
              {/* ABOUT WELFARE POLICY ADVICE MODULE */}
        ...
        // ==========================================
        // SINGLE POST ARTICLE COMPLETED VIEW PAGE
        // ==========================================
        <main className="flex-grow max-w-4xl mx-auto w-full px-4 sm:px-6 py-8">
          ...
          <article className="bg-white dark:bg-slate-950 rounded-3xl p-6 sm:p-10 border border-slate-205 dark:border-slate-850 shadow-xl overflow-hidden animate-fadeIn">
            
            {/* badge */}
            <div className="flex flex-wrap items-center gap-3 mb-4 text-xs font-black">
              <span className="bg-emerald-700 text-white hover:bg-emerald-850 px-3.5 py-1 rounded-full uppercase tracking-wider">
                {lang === "en" ? selectedItem.category : getBilingualCategory(selectedItem.category)}
              </span>
              <span className="text-slate-420 text-slate-400 capitalize">
            ...
            {/* Author Section */}
            <div className="flex items-center gap-3 my-6 py-4.5 py-4 border-y border-slate-150 dark:border-slate-850">
            ...
            <section className="bg-slate-50 dark:bg-slate-900/40 border border-slate-200 dark:border-slate-850 p-5 rounded-2xl mb-8">
            ...
            <div className="space-y-8 text-xs sm:text-sm leading-relaxed text-slate-705 dark:text-slate-350 font-sans">
              ...
              <div id="art-overview">
                ...
                <p className="font-sans leading-normal text-slate-650 dark:text-slate-300 font-medium">
              ...
              <div id="art-elig" className="pt-4">
                ...
                <ul className="space-y-2 pl-1">
                  {(lang === "en" ? selectedItem.eligibility : selectedItem.eligibilityAs).map((eligText, idx) => (
                    <li key={idx} className="flex items-start text-xs sm:text-sm text-slate-700 dark:text-slate-205 font-medium">
              ...
              {selectedItem.syllabus && (
                <div id="art-syllabus" className="pt-4 bg-slate-900 text-slate-100 p-5 rounded-2xl border border-slate-800 shadow-inner">
                  ...
                  <ul className="space-y-2 pl-1 font-mono text-[11px] text-slate-350">
            ...
            {/* DYNAMIC COMPREHENSIVE FAQ SYSTEM INTRO */}
            <section className="mt-12 pt-8 border-t border-slate-205 dark:border-slate-850">
              ...
              <div className="space-y-3.5">
                {[
                  ...
                  { q: "What should candidates carry to the exam centers?", qAs: "পৰীক্ষা কেন্দ্ৰলৈ কি কি সা-সুবিধা নিব লাগিব?", a: "Carry double print copies of the official Admit Card, passport photo, and an original PAN/Aadhaar/Voter card.", aAs: "ডাউনলোড কৰা প্ৰৱেশ পত্ৰৰ বিপৰীতে মূল পৰিচয় কাৰ্ড আৰু ফটো প্ৰয়োজন হব।" }
                ].map((faq, i) => (
                  <div key={i} className="border border-slate-200 dark:border-slate-800 rounded-2xl overflow-hidden bg-slate-50 dark:bg-slate-900/40">
                    <button 
                      ...
                      className="w-full text-left p-4.5 font-bold flex justify-between items-center text-slate-850 dark:text-slate-100 hover:bg-slate-100 cursor-pointer text-xs sm:text-sm font-sans"
                    >
                      <span>{lang === "en" ? faq.q : faq.qAs}</span>
                      <span>{activeFaqIndex === i ? "−" : "+"}</span>
                    </button>
                    {activeFaqIndex === i && (
                      <div className="p-4.5 border-t border-slate-200 dark:border-slate-805 bg-white dark:bg-slate-950 text-slate-650 dark:text-slate-350 text-xs sm:text-sm leading-relaxed font-sans font-medium animate-fadeIn">
            ...
            {/* RELATED PREVIEWS */}
            <section className="mt-10 pt-8 border-t border-slate-201 border-slate-200 dark:border-slate-850">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                {ASSAM_UPDATES.filter(u => u.id !== selectedItem.id).slice(0, 2).map((relatedItem) => {
                  const labelValue = lang === "en" ? relatedItem.title : relatedItem.titleAs;
                  return (
                    <div 
                      key={relatedItem.id}
                      onClick={() => { setSelectedItem(relatedItem); setActiveFaqIndex(null); }}
                      className="bg-slate-50 hover:bg-slate-100 dark:bg-slate-900 dark:hover:bg-slate-850 border border-slate-200 dark:border-slate-800 p-4.5 p-4 rounded-2xl cursor-pointer shadow-xs transition-all duration-200 flex flex-col justify-between"
                    >
            ...
            {/* SHARE ACTION SOCIAL BUTTONS */}
            <div className="mt-10 pt-8 border-t border-slate-200 dark:border-slate-850 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
              ...
              <div className="flex flex-wrap items-center gap-2">
                ...
                <button 
                  ...
                  className="p-2.5 bg-slate-100 dark:bg-slate-900 hover:bg-slate-200 dark:hover:bg-slate-800 text-slate-850 dark:text-slate-100 rounded-xl active:scale-95 transition-all cursor-pointer border border-slate-250 dark:border-slate-800"
            ...
            <div className="mt-8 bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-800 p-6 rounded-3xl flex flex-col sm:flex-row sm:items-center justify-between gap-4">
              <button 
                ...
                className="w-full sm:w-auto bg-[#001f3f] text-emerald-400 hover:text-white hover:bg-slate-950 font-bold px-5 py-3 rounded-xl text-xs flex items-center justify-center space-x-1.5 border border-slate-800 dark:bg-slate-950 dark:hover:bg-slate-900 transition-all cursor-pointer shadow-sm active:scale-95"
              >
              ...
              {selectedItem.officialLink && (
                <a 
                  ...
                  target="_blank"
                  rel="noreferrer"
                  className="w-full sm:w-auto bg-emerald-800 hover:bg-emerald-950 text-white font-extrabold px-6 py-3 rounded-xl text-xs tracking-wider flex items-center justify-center space-x-1.5 transition-all active:scale-95 text-center shadow-md shadow-emerald-700/10"
                >
      ...
      {/* FOOTER ACCORDING TO USER OBJECTIVES */}
      <footer className="bg-[#001f3f] dark:bg-slate-950 text-slate-300 py-12 px-4 sm:px-6 lg:px-8 border-t border-white/5 text-xs sm:text-sm transition-colors mt-auto">
        <div className="max-w-7xl mx-auto grid grid-cols-1 md:grid-cols-4 gap-8">
          ...
          <div className="space-y-3">
            ...
            <p className="text-xs text-slate-350 leading-relaxed font-sans font-medium">
              {lang === "en"
                ...
                : "অসম আৰু উত্তৰ-পূবৰ যুৱপ্ৰজন্মৰ সপোন বাস্তবায়নিত কৰাৰ পথত সহায় কৰা এক স্বতন্ত্ৰ তথ্যপূৰ্ণ আৰু পৰীক্ষা প্ৰস্তুতিৰ পৰ্টেল।"}
            </p>
          ...
          <div className="space-y-2">
            <h6 className="text-[11px] uppercase font-black tracking-widest text-[#FFF] text-[#2e7d32] dark:text-emerald-400">
              {lang === "en" ? "Contact Information" : "যোগাযোগ আৰু ঠিকনা"}
            </h6>
            <ul className="space-y-2 text-xs text-slate-350 font-medium font-sans">
          ...
          <div className="space-y-1.5">
            <h6 className="text-[11px] uppercase font-black tracking-widest text-emerald-450 text-[#fff]">
          ...
          {/* Government Policy Disclaimer Column */}
      ...
      {/* --- EXPORTER ZIP MULTI-TAB MODAL DIALOG SHEET --- */}
      {showExporterModal && (
        <div className="fixed inset-0 z-50 overflow-hidden flex items-center justify-center animate-fadeIn p-4">
          <div className="absolute inset-0 bg-slate-950/60 backdrop-blur-xs" onClick={() => setShowExporterModal(false)}></div>
          <div className="relative bg-white dark:bg-slate-950 rounded-3xl border border-slate-200 dark:border-slate-800 shadow-2xl max-w-2xl w-full h-[80vh] flex flex-col justify-between overflow-hidden">
            ...
            <div className="p-6 bg-[#001f3f] text-white flex items-center justify-between">
              <div>
                <span className="text-[10px] tracking-widest font-extrabold bg-yellow-605 bg-yellow-600 text-black py-0.5 px-2 rounded uppercase">Netlify Static Code Exporter</span>
              ...
              <button 
                ...
                className="bg-slate-800 hover:bg-slate-750 p-1.5 rounded-xl text-slate-205 cursor-pointer"
              >
                <X className="w-5 h-5 text-slate-100" />
              </button>
            </div>

            {/* Inner Tabs and Snippets */}
            <div className="p-6 flex-grow overflow-y-auto space-y-4">
              ...
              <div className="relative">
                ...
                <button 
                  ...
                  className="absolute top-2 right-2 bg-slate-800 hover:bg-slate-750 text-white font-extrabold p-2 rounded-xl text-[11px] flex items-center space-x-1.5 transition-all shadow-sm cursor-pointer"
                >
                  {copiedCodeCode ? <Check className="w-3.5 h-3.5 text-green-400" /> : <Copy className="w-3.5 h-3.5 text-slate-350" />}
                  <span>{copiedCodeCode ? "Copied!" : "Copy code"}</span>
                </button>
              </div>

              {/* File Info instructions list */}
              <div className="bg-slate-100 dark:bg-slate-900 border border-slate-205 dark:border-slate-800 p-4 rounded-xl space-y-2">
                <p className="text-[11px] font-extrabold uppercase text-[#001f3f] dark:text-emerald-400">Quick hosting instructions Guide:</p>
                <ol className="list-decimal pl-4 text-[11px] font-sans font-semibold text-slate-650 dark:text-slate-400 space-y-1 leading-normal">
      ...
      {/* --- SERVER PRESET REAL-TIME GEMINI API FLOATING CHAT --- */}
      <div className="fixed bottom-6 right-6 z-50 flex flex-col items-end">
        ...
        {chatOpen && (
          <div className="bg-white dark:bg-slate-950 text-slate-800 dark:text-slate-100 w-[92vw] sm:w-[420px] h-[550px] shadow-2xl rounded-3xl border border-slate-200 dark:border-slate-800 flex flex-col justify-between overflow-hidden mb-4 animate-slideUp">
              ...
              <div className="flex items-center space-x-3">
                ...
                <div>
                  ...
                  <p className="text-[10px] text-slate-400 font-extrabold flex items-center tracking-wider gap-1">
                    <Sparkles className="w-3 h-3 text-emerald-405 text-emerald-400 animate-spin-slow" />
            ...
            <div className="flex-grow overflow-y-auto p-4 space-y-4 bg-slate-50 dark:bg-slate-900/40 relative no-scrollbar">
                ...
                >
                  <div className={`max-w-[85%] rounded-2xl p-3.5 text-xs sm:text-xs font-semibold shadow-xs leading-normal font-sans ${
                    msg.role === "user" 
                      ...
                      : "bg-white text-slate-800 dark:bg-slate-950 dark:text-slate-100 border border-slate-200/80 dark:border-slate-850 rounded-bl-none whitespace-pre-wrap"
                  }`}>
                    <div>{msg.text}</div>
              ...
              {isTyping && (
                <div className="flex justify-start">
                  <div className="bg-white dark:bg-slate-950 border border-slate-200 dark:border-slate-850 rounded-2xl rounded-bl-none p-3.5 text-xs font-semibold text-slate-600 dark:text-slate-355 flex items-center space-x-2">
                    ...
                    <span className="text-[10px] uppercase font-bold tracking-widest text-[#001D00] dark:text-emerald-400">{lang === "en" ? "Xarothi AI is writing..." : "সাৰথি এআই এ লিখি আছে..."}</span>
                  </div>
                </div>
              )}
            ...
            {/* Preset Suggested query buttons */}
            <div className="p-2 border-t border-slate-150 dark:border-slate-850 bg-slate-100 dark:bg-slate-950/80 flex overflow-x-auto gap-1.5 no-scrollbar">
              <button 
                onClick={() => handleSendChat(undefined, "What is Orunodoi 3.0 eligibility criteria and monthly payout?")}
                className="text-[10px] font-bold bg-white dark:bg-slate-900 text-slate-705 dark:text-slate-300 border border-slate-205 dark:border-slate-800 px-3 py-1.5 rounded-full shrink-0 cursor-pointer active:scale-95 whitespace-nowrap"
              >
            ...
            {/* SEND FORM */}
            <form onSubmit={handleSendChat} className="p-3 bg-white dark:bg-slate-950 border-t border-slate-200 dark:border-slate-850 flex items-center space-x-2">
              <input 
                type="text"
                ...
                disabled={isTyping}
                className="flex-grow pl-3 pr-4 py-2 bg-slate-100 dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl text-xs sm:text-sm font-semibold focus:outline-none focus:bg-white focus:border-slate-400 dark:focus:bg-black text-slate-800 dark:text-slate-100 disabled:opacity-55"
              />
              <button 
                type="submit"
                disabled={isTyping}
                className="bg-[#001f3f] hover:bg-slate-950 text-emerald-400 p-2.5 rounded-xl border border-slate-800 dark:bg-slate-900 dark:hover:bg-slate-800 active:scale-95 shrink-0 transition-colors cursor-pointer disabled:opacity-50"
              >
        ...
        {/* BOTTOM ACTIVE PULSING LAUNCHER ICON */}
        <button 
          onClick={() => setChatOpen(!chatOpen)}
          className="bg-[#001f3f] hover:bg-slate-950 dark:bg-black text-white rounded-full p-4 sm:p-4.5 flex items-center justify-center shadow-2xl relative transform hover:scale-105 active:scale-95 border border-slate-800 cursor-pointer transition-all gap-2 group z-50"
        >
          {chatOpen ? (
            ...
            <div className="flex items-center space-x-2">
              ...
              <span className="text-xs font-black tracking-widest text-[#FFF] uppercase block font-sans">
